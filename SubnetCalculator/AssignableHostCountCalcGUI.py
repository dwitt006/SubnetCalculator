# Title:     Subnet Calculator: Assignable Host Count Calculator GUI
# Date:      04-16-2017
...
class AssignHostCalcGUI:
    def __init__(self):
        ...
        self.count_from_total = LabelFrame(self.assignable, text='Assignable Host Count: Total Host Count')
        self.total_label = Label(self.count_from_total, text='Total Host Count:', width=20)
        self.total_entry = Entry(self.count_from_total, width=25)

        self.count_from_cidr = LabelFrame(self.assignable, text='Assignable Host Count: CIDR Address')
        self.cidr_label = Label(self.count_from_cidr, text='CIDR Address:', width=20)
        self.cidr_entry = Entry(self.count_from_cidr, width=25)

        self.count_from_netmask = LabelFrame(self.assignable, text='Assignable Host Count: Netmask')
        ...
        self.count_from_ip_range = LabelFrame(self.assignable, text='Assignable Host Count: Assignable IP Range')
        ...
        self.learning_steps = LabelFrame(self.assignable, text='Learning Steps')

        self.calculate = Button(self.count_from_total, text='Calculate', command=lambda: self.get_count_from_total())
        self.calculate2 = Button(self.count_from_cidr, text='Calculate', command=lambda: self.get_count_from_cidr())
        self.calculate3 = Button(self.count_from_netmask, text='Calculate',
                                 command=lambda: self.get_count_from_netmask())
        self.calculate4 = Button(self.count_from_ip_range, text='Calculate',
    ...
    def get_count_from_total(self):
        if self.subnet.assignable_host_count != '':
            ...
            if self.subnet.verify_variables():
                ...
                count = Label(self.learning_steps, text='Assignable Host Count: {} - 2 = {}'.format(
                    self.subnet.total_host_count, self.subnet.assignable_host_count))
    ...
    def get_count_from_cidr(self):
        if self.subnet.assignable_host_count != '':
            ...
            if self.subnet.verify_variables():
                ...
                count = Label(self.learning_steps, text='Assignable Host Count: ( 2{} ) - 2 = {}'.format(
                    exponent(steps[1]['Host Bits'].split(' ')[-1]), self.subnet.assignable_host_count))
    ...
    def get_count_from_netmask(self):
        if self.subnet.assignable_host_count != '':
            ...
            if self.subnet.verify_variables():
                ...
                step4 = Label(self.learning_steps, text='Step 4:')
                count = Label(self.learning_steps, text='Assignable Host Count: ( 2{} ) - 2 = {}'.format(
                    exponent(str(steps[1]['Host Bits'])), self.subnet.assignable_host_count))
    ...
    def get_count_from_ip_range(self):
        if self.subnet.assignable_host_count != '':
            ...
            if self.subnet.verify_variables():
                ...
                count = Label(self.learning_steps, text='Assignable Host Count: ( 2{} ) - 2 = {}'.format(
                    exponent(str(steps[2]['Host Bits'])), self.subnet.assignable_host_count))
